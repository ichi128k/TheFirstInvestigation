...
namespace TFI.Vehicles
{
    ...
    public class TFIJetParticleEffect : MonoBehaviour
    {
		...
		private FrozenAeroplaneController m_Jet; // The jet that the particle effect is attached to
		...
		private FrozenAeroplaneController FindAeroplaneParent()
        {
            ...
            while (t != null)
            {
                var aero = t.GetComponent<FrozenAeroplaneController>();
                if (aero == null)
                {
                    // try next parent
                    t = t.parent;
			...
			throw new Exception(" FrozenAeroplaneController not found in object hierarchy");
        }
    }
}
