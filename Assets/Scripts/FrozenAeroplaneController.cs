...
namespace TFI.Vehicles
{
	...
	public class FrozenAeroplaneController : MonoBehaviour
	{
		...
		public float movingRangeX = 150;
		public float movingRangeY = 50;
		...
		private Vector3 oldPlaneRotation;
		...
		public void Move(float rollInput, float pitchInput, float yawInput, float throttleInput, bool airBrakes)
		{
			...
			//m_Rigidbody.MoveRotation(Quaternion.Euler(0,0,45));
			/*
			Debug.Log(m_Rigidbody.velocity.x);

			airPlaneObject.rotation = Quaternion.Euler(0,0,-45 * (m_Rigidbody.velocity.x / 100));
			oldRotationZ = airPlaneObject.eulerAngles.z;

			m_Rigidbody.velocity = new Vector3(RollInput * 100,PitchInput * 100,0);
			*/

			if(!GameManager.gameOver)
			{
				transform.position = new Vector3(Mathf.Clamp(transform.position.x,-movingRangeX,movingRangeX),
			                                 	Mathf.Clamp(transform.position.y,-movingRangeY,movingRangeY),
			                                	0);
			}
			else
			{
				transform.position = new Vector3(Mathf.Clamp(transform.position.x,-movingRangeX,movingRangeX),
				                                 Mathf.Clamp(transform.position.y,-200,movingRangeY),
				                                 0);
			}

			float eularAngleX = transform.eulerAngles.x;

			transform.rotation = Quaternion.Euler(RegulatePlaneRotation(transform.eulerAngles.x,oldPlaneRotation.x,45,315),
			                                      RegulatePlaneRotation(transform.eulerAngles.y,oldPlaneRotation.y,45,315),
			                                      transform.eulerAngles.z);

			oldPlaneRotation = transform.eulerAngles;

		}
		
		private float RegulatePlaneRotation(float value,float fixValue,float from,float to)
		{
